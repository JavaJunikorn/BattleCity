...
abstract class Moveable {
  ...
  Moveable(int x, int y, int this.width, int this.height,
      Directions this.direction, GameField this.gameField, int this.speed, String this.type) {
    print(gameField);
    ...
    for (int row = 0; row < height; row++) {
      ...
      for (int col = 0; col < width; col++) {
        ...
        gameField.getField(new Point(y +col, x +row)).moveable = this;
      }
    }
  }
  ...
  void move() {
    switch (direction) {
      case Directions.up:
        {
          positions[positions.length-1].forEach((p) => gameField.getField(p).moveable = null);

          _movePositions(Moveable.UP);

          positions[0].forEach((p) {
            gameField.getField(p).moveable = this;
          });
      ...
      case Directions.down:
        {
          positions[0].forEach((p) => gameField.getField(p).moveable = null);
      ...
      case Directions.left:
        {
          positions.forEach((l) => gameField.getField(l[l.length -1]).moveable = null);

          _movePositions(Moveable.LEFT);

          positions.forEach((l) {
            gameField.getField(l[0]).moveable = this;
      ...
      case Directions.right:
        {
          ...
          positions.forEach((l) => gameField.getField(l[0]).moveable = null);

          _movePositions(Moveable.RIGHT);

          positions.forEach((l) {
            gameField.getField(l[l.length -1]).moveable = this;
          });
