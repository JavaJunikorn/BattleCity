...
class View {
  ...
  TableElement toHTMLTable(Game game) {
    ...
    for (int i = 0; i < game.level.gamefield.height; i++) {
      ...
      for (int j = 0; j < game.level.gamefield.width; j++) {
        ...
        tBody.rows.elementAt(i).cells.elementAt(j).setAttribute("class", "bg-" + game.level.gamefield.gameField[i+1][j+1].ground.type);
      }
    }

    for(int i = 0; i < game.level.gamefield.moveables.length; i++){
      tBody.rows.elementAt(game.level.gamefield.moveables[i].positions[0][0].y).cells.elementAt(game.level.gamefield.moveables[i].positions[0][0].x).setAttribute("class", "bg-" + game.level.gamefield.moveables[i].type);
    }
  ...
  void update(int delay) {
    ...
    new Timer.periodic(new Duration(milliseconds: delay), (t){
      htmlTable = querySelector(".main-container").children.first.children.first;
      rows = htmlTable.children;
      for(int i = 0; i < rows.length; i++){
        cols = rows[i].children;
        for(int j = 0; j < cols.length; j++){
          cols[j].setAttribute("class", "bg-" + model.level.gamefield.gameField[i+1][j+1].ground.type);
        }
      }
  ...
  void setAttributeTo(Point destination, String name, String value) {
    rows.elementAt(destination.y).children.elementAt(destination.x).setAttribute(name, value);
  }
